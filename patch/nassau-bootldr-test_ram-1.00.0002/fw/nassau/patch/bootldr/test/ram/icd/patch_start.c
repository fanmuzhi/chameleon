...
/*
 * Test the instruction cache data RAM.
 *
 * TODO:
 * + switch to using an isolated-mapping version of the instruction
 *   cache data memory.  This should allow for a more robust test
...
patch_start(void *ctxp, uint8_t *replyp)
{
    ...
    for (passno = 0; passno < PATCH_NPASSES; passno++) {
        if (!PATCH_RAMTEST((uint32_t *) RAMTEST_ICT_PSEUDOADDR,
                           XCHAL_ICACHE_SIZE / sizeof(uint32_t),
