                ...
                unsigned int cmdlen, bool_t overflow)
{
    ...
    if (overflow) {
        ...
        for (idx = 0; idx < csp->nentries; idx++) {
            if (csp->tablep[idx].id == (uint32_t) cmdbyte) {
         ...
         *  Note that the current patch could have installed
         *  a handler for VCSFW_CMD_UNLOAD_PATCH and it would
         *  have been run already.  This just completes the
         *  function.
         */
        if (cmdbyte == VCSFW_CMD_UNLOAD_PATCH) {
#if defined(CMDMGR_OPT_CMDSET) && defined(CMDMGR_OPT_DIRSPI)
            if (csp->use_cmdset) {
