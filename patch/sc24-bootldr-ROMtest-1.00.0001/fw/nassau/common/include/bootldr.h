...
#if !defined(NASSAU_PRODUCT_SC24PBL)
/* This value taken from vcsfw_v4.h */
#   define NASSAU_PRODUCT_SC24PBL   64
#endif
 ...
 * product below.  This is added to simplify patch building. MJW20160603 */
    ...
    && NASSAU_PRODUCT != NASSAU_PRODUCT_SC24
#   error "NASSAU_PRODUCT must be one of NASSAU_PRODUCT_{DENALI,HAYES,DENALIA,SHASTA,SC24}"
#endif
...
#if !defined(NASSAU_PATCH_BOOTLDR_IFVER) || NASSAU_PATCH_BOOTLDR_IFVER >= 4
/* ifver=4: */
#define BOOTLDR_FUNCPS_INDEX_SECSTATE_GET       18
#define BOOTLDR_FUNCPS_INDEX_TABLE_GET          19
#endif      /* !defined(NASSAU_PATCH_BOOTLDR_IFVER)
             * || NASSAU_PATCH_BOOTLDR_IFVER >= 4 */
...
/*
 * Values for SECSTATE_GET:
 */
    /*
     * BOOTLDR_SECSTATE_OPEN.  Sensor is not secured and can
     *  accept unsigned patches.
     */
#define BOOTLDR_SECSTATE_OPEN               0

    /*
     * BOOTLDR_SECSTATE_ENG.  Sensor is a secured engineering
     *  sensor.  Will only accept encrypted/signed patches with
     *  engineering keys.
     */
#define BOOTLDR_SECSTATE_ENG                1

    /*
     * BOOTLDR_SECSTATE_PROD.  Sensor is a secured production
     *  sensor.  Will only accept encrypted/signed patches with
     *  production keys.
     */
#define BOOTLDR_SECSTATE_PROD               2

    /*
     * BOOTLDR_SECSTATE_LOCKED.  Sensor is locked up because of an
     *  invalid security word.  Will only accept encrypted/signed patches
     *  with production keys.
     */
#define BOOTLDR_SECSTATE_LOCKED             3
    /*
     * BOOTLDR_SECSTATE_PROG.  Hayes (b1212) / SC24 (b2224) only.
     *
     * Sensor is *un*programmed (sorry about the
     *  name -- blame the hardware people).  This can mean one of two
     *  things, depending on if the fuse_default_open, which is
     *  a hardware metal option, is set or cleared.
     * If fuse_default_open = 1, then the part is completely open and
     *  all operations are possible.  This is how the Hayes A0 part
     *  will be configured.
     * If fuse_default_open = 0, then the only possible action is for
     *  the part to program fuses.  Note that the bootldr should
     *  never run with the part in this state, as an ATE patch should
     *  erase the entire flash and program the initial fuse state.
     *
     * The bootldr in this state acts the same as in BOOTLDR_SECSTATE_OPEN.
     */
#define BOOTLDR_SECSTATE_PROG               4

    /*
     * BOOTLDR_SECSTATE_FA.  Hayes (b1212) / SC24 (b2224) only.
     *
     * Failure analysis mode.
     *  Everything is open except the PUF is inaccessible.
     *
     * The bootldr in this state acts the same as in BOOTLDR_SECSTATE_OPEN.
     */
#define BOOTLDR_SECSTATE_FA                 5


/*
 * Values for TABLE_GET:
 */
/* AES forward S-box (8bit -> 8bit) */
#define BOOTLDR_TABLEID_AES_SBOX            0
/* AES inverse S-box (8bit -> 8bit) */
#define BOOTLDR_TABLEID_AES_ISBOX           1
/* AES inverse round constants (10 entries -> 8bit) */
#define BOOTLDR_TABLEID_AES_RCON            2
/* SHA-256 kx table (6bit -> 32bit) */
#define BOOTLDR_TABLEID_SHA256_KX           3
/* SHA-256 init table (8 elements -> 32bit) */
#define BOOTLDR_TABLEID_SHA256_INIT         4
/* PRESENT forward S-box (4bit -> 8bit (actually 4 bit) */
#define BOOTLDR_TABLEID_PRESENT_SBOX        5
/* PRESENT inverse S-box (4bit -> 8bit (actually 4 bit) */
#define BOOTLDR_TABLEID_PRESENT_ISBOX       6
/* PRESENT forward bit permutation list (6bit -> 8bit (actually 6bit)) */
#define BOOTLDR_TABLEID_PRESENT_BITPERMUT   7
/* PRESENT inverse bit permutation list (6bit -> 8bit (actually 6bit)) */
#define BOOTLDR_TABLEID_PRESENT_IBITPERMUT  8
/* PRESENT forward double S-box (8bit -> 8bit) */
#define BOOTLDR_TABLEID_PRESENT_DSBOX       9
/* PRESENT inverse double S-box (8bit -> 8bit) */
#define BOOTLDR_TABLEID_PRESENT_IDSBOX      10
/* PRESENT forward collapsed bit permut + S-box (8-bit -> 64 bit) */
#define BOOTLDR_TABLEID_PRESENT_CPS8        11
/* PRESENT inverse bit permutation (8bit -> 32bit) */
#define BOOTLDR_TABLEID_PRESENT_IP8         12
...
#if !defined(NASSAU_PATCH_BOOTLDR_IFVER) || NASSAU_PATCH_BOOTLDR_IFVER >= 4
    /*
     * This is additional for ifver=4:
     */
    unsigned int    (*secstate_get)(void *ctxp);
    const void *    (*table_get)(unsigned int tableid);
#endif      /* !defined(NASSAU_PATCH_BOOTLDR_IFVER)
             * || NASSAU_PATCH_BOOTLDR_IFVER >= 4 */
...
#if !defined(NASSAU_PATCH_BOOTLDR_IFVER) || NASSAU_PATCH_BOOTLDR_IFVER >= 4
BOOTLDR_ASSERT_CT(bootldr_funcps_index_secstate_get_valid,
                  OFFSETOF(bootldr_funcps_t, secstate_get)
                  == (BOOTLDR_FUNCPS_INDEX_SECSTATE_GET
                      * sizeof(void *)));
BOOTLDR_ASSERT_CT(bootldr_funcps_index_table_get_valid,
                  OFFSETOF(bootldr_funcps_t, table_get)
                  == (BOOTLDR_FUNCPS_INDEX_TABLE_GET
                      * sizeof(void *)));
#endif      /* !defined(NASSAU_PATCH_BOOTLDR_IFVER)
             * || NASSAU_PATCH_BOOTLDR_IFVER >= 4 */
...
#if !defined(NASSAU_PATCH_BOOTLDR_IFVER) || NASSAU_PATCH_BOOTLDR_IFVER >= 4
#define BOOTLDR_SECSTATE_GET(ctxp)                                          \
    (*bootldr_funcps.secstate_get)((ctxp))
#define BOOTLDR_TABLE_GET(tableid)                                          \
    (*bootldr_funcps.table_get)((tableid))
#endif      /* !defined(NASSAU_PATCH_BOOTLDR_IFVER)
             * || NASSAU_PATCH_BOOTLDR_IFVER >= 4 */
