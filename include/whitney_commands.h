...
#define WFW_EVENT_TYPE_SB_FORCE_PRESS     0x17
#define WFW_EVENT_TYPE_SB_FORCE_RELEASE   0x18
...
/* The command: */
typedef struct WFW_PACKED wfw_cmd_get_frame_data_s {
...
} wfw_cmd_get_frame_data_t;

/* The reply is only the generic reply followed by some number of ADC values */
...
#define WFW_IOTA_ITYPE_CONFIG_TDL         0x000F
...
typedef struct WFW_PACKED wfw_iota_config_tap_detector_s {
    wfwUint8_t max_tap_time_ms;
    wfwUint8_t max_dtap_gap_time_ms;
    wfwUint8_t min_longpress_time_ms;
 ...
 *        buttons.
 *  - Absolute: Represents an absolute
 */
...
typedef struct WFW_PACKED wfw_reply_get_sys_info_s {
  ...
  wfwSint16_t wofFP_fdAbsThresh; //fu threshold actually written to the hardware
